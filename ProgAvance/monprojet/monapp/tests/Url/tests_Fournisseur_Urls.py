...
from monapp.views import FournisseurCreateView, FournisseurListView, FournisseurDetailView, FournisseurUpdateView, FournisseurDeleteView
...
class FournisseurTestUrls(SimpleTestCase):
    ...
    def test_delete_view_url_is_resolved(self):
        """
        Tester que l'URL de suppression de Fournisseur renvoie la bonne vue
        """
        url = reverse('fournisseur-delete', args=[1])
...
class FournisseurTestUrlRedirect(TestCase):
    ...
    def test_redirect_after_creation(self):
        ...
        if response.status_code == 200:
            print(response.context['form'].errors)  # Affiche les erreurs de validation du formulaire si présent
